...
export default {
  ...
  methods: {
    ...
    exportToExcel() {
      const worksheet = XLSX.utils.json_to_sheet(this.incomingApplications)
    ...
    editSession(item) {
      this.editedIndex = this.incomingApplications.indexOf(item)
  ...
  computed: {
    filteredincomingApplications() {
      return this.incomingApplications.filter(session => {
        return (
