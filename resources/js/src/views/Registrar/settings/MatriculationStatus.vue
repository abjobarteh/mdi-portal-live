      ...
      <v-container fluid>
        <v-card>
          <v-toolbar color="primary" dark>
            <v-toolbar-title>Matriculation Status</v-toolbar-title>
            <v-spacer></v-spacer>
          </v-toolbar>
  ...
  export default {
    ...
    methods: {
      ...
      confirmSwitch(item) {
        ...
        swal
          .fire({
            title: `Are you sure you want to ${action} Matriculation?`,
            icon: 'question',
          ...
          .then(result => {
            if (result.isConfirmed) {
                console.log('Val: ',item.switchValue)
      ...
      setSwitchState() {
        if (this.admissionStatus.length === 1) {
          ...
          if (item.matriculation_status == 1) {
            item.switchValue = true // Set switchValue to true for open admission
          } else if (item.matriculation_status == 0) {
      ...
      openAdmission(item) {
        ...
        axios
          .post('/api/update-matriculation', { status: 'Open' })
          .then(response => {
            // Handle the API response if needed
            console.log(`Matriculation opened for ${item.status}`)
            swal
              .fire({
                title: 'Success!',
                text: 'Matriculation Open successfully.',
                icon: 'success',
          ...
          .catch(error => {
            ...
            console.error('Failed to open Matriculation:', error)
        ...
        if (index !== -1) {
          ...
          console.error('Failed to find item in MatriculationStatus')
          // Revert the switch value if the action failed
          item.switchValue = !item.switchValue
        }
      },
      ...
      closeAdmission(item) {
        ...
        // Example API call using axios:
        axios
          .post('/api/update-matriculation', { status: 'Close' })
          .then(response => {
            // Handle the API response if needed
            ...
            swal
              .fire({
                title: 'Success!',
                text: 'Matriculation closed successfully.',
                icon: 'success',
                confirmButtonText: 'OK',
              })
              .then(() => {
          ...
          .catch(error => {
            // Handle any errors that occurred during the API call
            console.error('Failed to close Matriculation:', error)
            // Revert the switch value if the action failed
            item.switchValue = !item.switchValue
          })
        ...
        if (index !== -1) {
          ...
          console.log(`Matriculation closed for ${item.status}`)
        } else {
          console.error('Failed to find item in MatriculationStatus')
          // Revert the switch value if the action failed
          item.switchValue = !item.switchValue
